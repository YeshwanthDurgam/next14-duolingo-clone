...
import { ArrowDownAZ, Award } from "lucide-react";
...
export const getLesson = cache(async (id?: number) => {
    ...
    const normalizedChallenges = data.challenges.map((challenge) => {
        const completed = challenge.challengeProgress && challenge.challengeProgress.length > 0 && challenge.challengeProgress.every((proogess) => proogess.completed);
